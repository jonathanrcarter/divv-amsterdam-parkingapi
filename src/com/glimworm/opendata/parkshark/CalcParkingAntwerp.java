...
public class CalcParkingAntwerp extends CalcParking {
...
//	}	
	...
	public static PayTimes getcosts(Meter meter) {
		...
		*/
		...
		// this is the normal
		retval.days[0] = decodeDate("");
		retval.days[1] = decodeDate("09:00-19:00");
		retval.days[2] = decodeDate("09:00-19:00");
		retval.days[3] = decodeDate("09:00-19:00");
		retval.days[4] = decodeDate("09:00-19:00");
		retval.days[5] = decodeDate("09:00-19:00");
		retval.days[6] = decodeDate("09:00-19:00");
		
		if (meter.tariefcode.equalsIgnoreCase("A11")) {
			retval.days[0] = decodeDate("");
			retval.days[1] = decodeDate("09:00-22:00");
			retval.days[2] = decodeDate("09:00-22:00");
			retval.days[3] = decodeDate("09:00-22:00");
			retval.days[4] = decodeDate("09:00-22:00");
			retval.days[5] = decodeDate("09:00-22:00");
			retval.days[6] = decodeDate("09:00-22:00");
			retval.max = 240;
			retval.first.combination = "tripple";
			retval.first.hrs = 1;
			retval.first.price = 1.5;
			retval.first.hrs2 = 1;
			retval.first.price2 = 2.5;
			retval.cost = 3.5;
			
		} else if (meter.tariefcode.equalsIgnoreCase("A12")) {
			retval.days[0] = decodeDate("");
			retval.days[1] = decodeDate("09:00-22:00");
			retval.days[2] = decodeDate("09:00-22:00");
			retval.days[3] = decodeDate("09:00-22:00");
		...
		} else if (meter.tariefcode.equalsIgnoreCase("B11")) {
			// light green with NO day tarrif + late payment
			retval.days[0] = decodeDate("");
			retval.days[1] = decodeDate("09:00-22:00");
			retval.days[2] = decodeDate("09:00-22:00");
			retval.days[3] = decodeDate("09:00-22:00");
			...
			retval.first.combination = "y";
			retval.first.hrs = 1;
			retval.first.price = 0.6;
			retval.cost = 1.0;
			retval.max = 600;
		} else if (meter.tariefcode.equalsIgnoreCase("B21")) {
			// light green with NO day tarrif + NO late payment
			retval.first.combination = "y";
			retval.first.hrs = 1;
			retval.first.price = 0.6;
			retval.cost = 1.0;
			retval.max = 600;
		} else if (meter.tariefcode.equalsIgnoreCase("B22")) {
			// light green with NO day tarrif + NO late payment
			retval.first.combination = "y";
			retval.first.hrs = 1;
			retval.first.price = 0.6;
			retval.cost = 1.0;
			retval.max = 600;
		} else if (meter.tariefcode.equalsIgnoreCase("C11")) {
			// dark green with day tarrif + late payment
			retval.days[0] = decodeDate("");
			retval.days[1] = decodeDate("09:00-22:00");
			retval.days[2] = decodeDate("09:00-22:00");
			retval.days[3] = decodeDate("09:00-22:00");
			...
			retval.cost = 1.0;
			retval.max = 600;
			retval.maxdaycost = 3.5;
		} else if (meter.tariefcode.equalsIgnoreCase("C21")) {
			// dark green with day tarrif
			retval.first.combination = "y";
			retval.first.hrs = 1;
			retval.first.price = 0.6;
			retval.cost = 1.0;
		...
		} else if (meter.tariefcode.equalsIgnoreCase("D21")) {
	...
	public static void populate_meters() {
		String sql = "select * from _site1493_dbsynch_automats_antwerp";
		...
		for (int i=0; i < automats.rows(); i++) {
			...
			/*
			mysql> select * from _site1493_dbsynch_automats_antwerp limit 10,1\G
			*************************** 1. row ***************************
			        PA_C_254: 23807			0
			    Tarief_C_254: D22			1
			 T_BK_Prog_C_254: 10;99;38;		2
			    Straat_C_254: Abdijstraat	3
			Huisnummer_C_254: 162			4
			    Gebied_C_254: Antwerpen		5
			        X_N_16_6: 4.385000		6
			        Y_N_16_6: 51.187000		7
			    Plaats_C_254: Antwerpen		8
			1 row in set (0.00 sec)
			*/
			
			smeters[i] = new Meter();
			smeters[i].i = i;
			smeters[i].entityid = automats.getString(i, 0);
			smeters[i].stadsdeel = automats.getString(i, 5);
			smeters[i].belnummer = automats.getString(i, 0);
			smeters[i].adres = automats.getString(i, 3)+ " " + automats.getString(i, 4);
			smeters[i].postcode = "";
			smeters[i].woonplaats = automats.getString(i, 8);
			smeters[i].typeautomaat = "";
			smeters[i].betaalwijze = "";
			smeters[i].tariefcode = automats.getString(i, 1);
			smeters[i].status = "";
			smeters[i].lat = automats.getDouble(i, 7,0);
			smeters[i].lon = automats.getDouble(i, 6,0);

			smeters[i].bw.cash = true;
			smeters[i].bw.creditcard = true;
			smeters[i].bw.pin = true;
			smeters[i].bw.chip = false;
...
//			if (meter.match == 0) continue;
			...
			for (int j=0; j < days.size(); j++) {
				...
				int el = 0;
				double dayval = 0;
				if (costs.first.combination.equalsIgnoreCase("tripple") && j == 0) {
					if (dayhours <= costs.first.hrs) {
						dayval += (dayhours * costs.first.price);
						if (DBG) dbg += "f\n";
						if (DBG) dbg += "[yj:"+j+"/s:"+day.start+"/e:"+day.end+"/c:"+costs.cost+"/hrs:"+dayhours+"(all in first hours),dayval="+dayval+"]\n";
					} else if (dayhours <= (costs.first.hrs + costs.first.hrs2)) {
						dayval += (costs.first.hrs * costs.first.price);
						dayval += ((dayhours - costs.first.hrs) * costs.first.price2);
						if (DBG) dbg += "f\n";
						if (DBG) dbg += "[yj:"+j+"/s:"+day.start+"/e:"+day.end+"/c:"+costs.cost+"/hrs:"+dayhours+"(all in first hours),dayval="+dayval+"]\n";
					} else {
						dayval += (costs.first.hrs * costs.first.price);
						dayval += (costs.first.hrs2 * costs.first.price2);
						dayval += ((dayhours - costs.first.hrs - costs.first.hrs2) * costs.cost);
	
						if (DBG) dbg += "f\n";
						if (DBG) dbg += "[yj:"+j+"/s:"+day.start+"/e:"+day.end+"/c:"+costs.cost+"/hrs:"+dayhours+"(some in first hours),dayval="+dayval+"]\n";
					}
					
				} else if (costs.first.combination.equalsIgnoreCase("y") && j == 0) {
					if (dayhours <= costs.first.hrs) {
						dayval += (dayhours * costs.first.price);
						if (DBG) dbg += "f\n";
						if (DBG) dbg += "[yj:"+j+"/s:"+day.start+"/e:"+day.end+"/c:"+costs.cost+"/hrs:"+dayhours+"(all in first hours),dayval="+dayval+"]\n";
						...
						dayval += (costs.first.hrs * costs.first.price);
						dayval += ((dayhours - costs.first.hrs) * costs.cost);
	
						if (DBG) dbg += "f\n";
						if (DBG) dbg += "[yj:"+j+"/s:"+day.start+"/e:"+day.end+"/c:"+costs.cost+"/hrs:"+dayhours+"(some in first hours),dayval="+dayval+"]\n";
					}
				} else {
					dayval += (dayhours * costs.cost);
					if (DBG) dbg += "f\n";
					if (DBG) dbg += "[yj:"+j+"/s:"+day.start+"/e:"+day.end+"/c:"+costs.cost+"/hrs:"+dayhours+"(none in first hours),dayval="+dayval+"]\n";
				}
				if (costs.maxdaycost > 0 && costs.maxdaycost > dayval) dayval = costs.maxdaycost;
...
//			
			int match = 1;
			if (meter.max > 0 && total_parking_minutes > meter.max) match = 0;
			if (meter.costs.error != 0) match = 0;
...
//			System.out.println("d:"+meters[i].dist + "c:"+meters[i].cost + "a:"+meters[i].adres+" \n"+meters[i].dbg+"\n");
			...
			if (meters[i].match == 0) continue nextmeter;
